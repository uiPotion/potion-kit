...
function formatChatError(err: unknown): string {
  ...
  if (
    /not a chat model|v1\/chat\/completions|v1\/completions/i.test(msg)
  ) {
    return (
      msg +
...
function createProgressReporter(): { start: () => void; onProgress: CreateChatOptions['onProgress']; clear: () => void } {
